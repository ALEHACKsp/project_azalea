...
ERR_CODE fat_filesystem::fat_folder::create_child(const kl_string &name, std::shared_ptr<ISystemTreeLeaf> &child)
{
  ...
  else if (populate_short_name(name, short_name))
  {
    KL_TRC_TRACE(TRC_LVL::FLOW, "Creating short name: ", short_name, "\n");
    is_long_name = false;

    kl_memcpy(short_name, &file_entries[0].name, 11);
    file_entries[0].attributes.archive = 1;

    num_file_entries = 1;

    result = ERR_CODE::NO_ERROR;
  }
  else if(populate_long_name(name, file_entries, num_file_entries))
  {
    KL_TRC_TRACE(TRC_LVL::FLOW, "Creating long name.\n");
    is_long_name = true;

    if (!generate_basis_name_entry(name, file_entries[num_file_entries]))
    {
    ...
    {
      result = ERR_CODE::NO_ERROR;
      checksum = generate_short_name_checksum(file_entries[num_file_entries].name);
      for (uint8_t i = 0; i < num_file_entries; i++)
      {
        KL_TRC_TRACE(TRC_LVL::FLOW, "Set checksum on entry ", i, "\n");
        file_entries[i].long_fn.checksum = checksum;
      }
      num_file_entries++;
    }
  }
  ...
  if (result == ERR_CODE::NO_ERROR)
  {
    ...
    result = add_directory_entries(file_entries, num_file_entries);
  }
...
ERR_CODE fat_filesystem::fat_folder::add_directory_entries(fat_dir_entry *new_fdes, uint8_t num_entries)
{
